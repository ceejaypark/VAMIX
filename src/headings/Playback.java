...
public class Playback {
	...
	private JButton _jbChooseSub;
	private JTextField _subSelection;
	...
	public void insertGUIFeatures(final JFrame frame1, final JTabbedPane pane2, final JTabbedPane pane3, final JTabbedPane pane){	
		...
		layout.putConstraint(SpringLayout.WEST, text, 5, SpringLayout.EAST, _jbChooseVid);
		layout.putConstraint(SpringLayout.NORTH, text, 2, SpringLayout.NORTH, _jbChooseVid);

		//move the video file choice button to its location
		layout.putConstraint(SpringLayout.WEST, _jbChooseVid, 15, SpringLayout.WEST, pane);
		layout.putConstraint(SpringLayout.NORTH, _jbChooseVid, 45, SpringLayout.NORTH, pane);

		//move the audio file choice button to its location
		layout.putConstraint(SpringLayout.WEST, _jbChooseSub, 15, SpringLayout.WEST, pane);
		layout.putConstraint(SpringLayout.NORTH, _jbChooseSub, 5, SpringLayout.SOUTH, _jbChooseVid);

		//move the audio text box to it's location
		layout.putConstraint(SpringLayout.WEST, _subSelection, 5, SpringLayout.EAST, _jbChooseSub);
		layout.putConstraint(SpringLayout.NORTH, _subSelection, 2, SpringLayout.NORTH, _jbChooseSub);

		//move the start button to its location
		layout.putConstraint(SpringLayout.WEST, jbBegin, 15, SpringLayout.WEST, pane);
		layout.putConstraint(SpringLayout.NORTH, jbBegin, 15, SpringLayout.SOUTH, _jbChooseSub);
	...
	private void addFileChoice(){
		//creates new button
		_jbChooseVid = new JButton("Choose File");
		...
		_jbChooseVid.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				jfile.addChoosableFileFilter(new FileNameExtensionFilter("AVI, MPG, WMV, MP3 or MP4 files", "avi", "mpg", "wmv", "mp4","mp3"));
		...
		addButtonToPane(panel,_jbChooseVid);

	}

	//adds text box to inform user which file they have chosen
	private void addSubSelectionBox(){
		//add text field to show which file chosen
		_subSelection = new JTextField();
		_subSelection.setEditable(false);
		panel.add(_subSelection);
		_subSelection.setPreferredSize(new Dimension(400,20));
	}

	//adds file choice button and implementation
	private void addSubFileChoice(){
		//creates new button
		_jbChooseSub = new JButton("Choose Subtitles");

		//adds file choosing functionality to button
		_jbChooseSub.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// Opens JFileChooser when button pressed
				JFileChooser jfile = new JFileChooser();
				// remove the accept all filter
				jfile.setAcceptAllFileFilterUsed(false);
				// filters results so only video files can be added
				jfile.addChoosableFileFilter(new FileNameExtensionFilter("SRT File", "srt"));
				int response = jfile.showOpenDialog(null);
				if (response == JFileChooser.APPROVE_OPTION) {
					String chosenFile = jfile.getSelectedFile().toString();
					_subSelection.setText(chosenFile);
				}
				jfile.setVisible(true);
			}
		});

		addButtonToPane(panel,_jbChooseSub);
...
mediaPlayerComponent.getMediaPlayer().setRepeat(true);
		...
		String srtFile = _subSelection.getText();
		video.setSubTitleFile(srtFile);
	...
	private void addVolumeControl(){
		...
		volumeControl.setToolTipText("Change volume");
	...
	private void addTimer(){
		...
		//start timer
		time.start();
if(video.getLength() == video.getTime() ){
